...
- 현재 단계: **M3·M4 진행 중** — slice-1~10a 완료 (transcript / OTel / hook /
  ...
  transcript-only file lineage (slice-10a)**).
- 남은 작업: M3 잔여(episode segmentation, causal inference, file lineage
  graph linkage), M4 잔여 (Why Panel, Source views, Resource drawer),
  **M5 Insight engine (0건 finding)**, M6 MCP Streamable HTTP, M7 보안
  강화 (Origin/token/redaction manifest/retention).
