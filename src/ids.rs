...
/// Derive a deterministic `finding_id` from category, session, and sorted evidence refs.
/// Format: `"find_" + hex(sha256(category + "\0" + session_id + "\0" + sorted_refs.join(",")))[..24]`.
pub fn derive_finding_id(category: &str, session_id: &str, evidence_refs: &[String]) -> String {
...
impl MonotonicUlidGen {
    ...
    pub fn generate(&mut self) -> String {
        ...
        self.inner
            .generate()
            .expect("ulid generator overflow")
            .to_string()
    }
}
...
mod tests {
    ...
    fn derive_node_id_is_deterministic() {
        let a = derive_node_id(
            "tool_call",
            &[("session_id", "s1"), ("tool_use_id", "toolu_abc")],
        );
        let b = derive_node_id(
            "tool_call",
            &[("tool_use_id", "toolu_abc"), ("session_id", "s1")],
        );
