...
export const ko: Messages = {
  ...
  'filter.content.toolPlaceholder': '다른 도구명 직접 입력 + Enter',
  'filter.content.modelPlaceholder': '다른 모델 ID 직접 입력 + Enter',
  'filter.content.toolsInSession': '이 세션의 도구',
  'filter.content.modelsInSession': '이 세션의 모델',
  'filter.content.mcpGroupTip': (server: string) => `${server} 서버의 MCP 도구 전체를 한 번에 켜고 끕니다`,
  'filter.qPlaceholder': '텍스트 검색 — 메시지 본문·도구 입력/결과…',
