...
async fn make_server_with_session() -> (TestServer, String) {
    ...
    let server = TestServer::new(witmcc::api::router(state)).unwrap();
    (server, sid)
}
...
#[tokio::test]
async fn resources_read_graph_carries_redaction_annotation() {
    let (server, session_id) = make_server_with_session().await;
    let mcp_sid = init_mcp_session(&server).await;
    let (hk, hv) = sid_header(&mcp_sid);
    ...
    let r = server
        ...
        .json(&json!({
            ...
            "method": "resources/read",
            "params": {
                "uri": format!("whats-in-my-cc://sessions/{session_id}/graph")
            }
        }))
        .await;
    ...
    assert!(
        ann["redaction_policy"].is_object(),
        "annotations.redaction_policy must be present on graph resource"
    );
    assert!(
        ann["redaction_summary"].is_object(),
        "annotations.redaction_summary must be present on graph resource"
    );
}
