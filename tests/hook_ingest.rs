...
// The three hook events are accepted into observed_event (SSOT) and remain
// observable via the events endpoint.
#[tokio::test]
async fn batch_three_ingests_all_to_observed_event() {
    let s = setup().await;
