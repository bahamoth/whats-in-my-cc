...
async fn health_sources_returns_full_taxonomy_when_db_empty() {
    ...
    let labels: Vec<&str> = sources
        ...
        .collect();
    // Slice-10a — "file-git" label removed. 2026-06-19 — "hook" label removed
    // with the hook collector. File lineage comes from transcript
    // toolUseResult.structuredPatch; remaining taxonomy is transcript + 3 OTel.
    assert_eq!(
        labels,
        vec!["transcript", "otel-traces", "otel-metrics", "otel-logs"]
    );
    for src in sources {
