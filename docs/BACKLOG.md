...
## 2026-07-04 대시보드 개편 후속

- **B-12 리플레이 스트림 지시문 변경 마커**: instruction_observation의 관측
  시각을 스트림 그룹핑 파이프라인에 파생 항목으로 삽입(Derived 배지) — 카드/
  diff는 구현됨, 인라인 서사 마커만 남음.
- **B-13 Tier3 지시문 존재 스캔**: cwd 트리 하위 CLAUDE.md 나열·스냅샷(존재
  기록만, 로드 무주장). 스캔 비용·노이즈 검토 후.
- **B-14 코호트 EXCEED 임계의 차원 수 보정**: 개입 차원이 늘면 다중비교로
  우연 상위가 늘어난다 — 임계 0.10의 차원 수 감응 보정 검토(스펙 §2 상수 개정).
