...
구현은 영역별 독립 PR로 분할한다(§5 — 단, 그 4-PR 분할은 머지 취합 비용이 커
사후 반례로 판명됐다; 앞으로는 단일 PR 취합, `CLAUDE.md` Operations "PR 단위"). 표기 원칙은 대시보드 스펙
...
> **⚠️ 사후 교훈 (2026-07-05):** 이 4-PR 분할은 각 PR의 베이스가 rebase 머지될 때
> SHA가 재작성돼 append-only 원장(`implementation-notes`·`notes-index`)·태깅 사전
> (`event_tags`·`tagging-gate-baseline`)·공유 파일(`SessionDetailPage`)에서 반복
> 충돌을 일으켰고, 머지 취합 비용이 구현 비용에 맞먹었다. worktree 격리나 강한
> 멀티에이전트 병렬 없이는 특히 비효율. **앞으로 관련 기능은 단일 PR로 취합한다**
> — 지침 SSOT는 `CLAUDE.md` Operations "PR 단위". 이 §5는 그 반례로 남긴다.
